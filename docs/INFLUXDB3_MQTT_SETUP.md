# MQTT → InfluxDB 3 Core Integration via Telegraf
...
Mosquitto (MQTT Broker)
    ↓ (subscribes to homeassistant/sensor/+/state)
...
```bash
echo "INFLUXDB3_ADMIN_TOKEN=apiv3_EXAMPLE_TOKEN_REPLACE_WITH_YOUR_ACTUAL_TOKEN_VALUE_HERE" >> .env
```
...
docker compose exec mosquitto mosquitto_pub \
  ...
  -m '{"state": "22.5", "attributes": {"unit_of_measurement": "°C", "friendly_name": "Living Room Temperature"}}'
```

### 6. Query Real Device Data (ESP sensors)

Device sensors publish directly to MQTT and are written into the `temperature_data` database. Try:

1. Open `http://localhost:8888`
2. Select `temperature_data`
3. Run:
  ```sql
  SELECT time, device, celsius, fahrenheit FROM esp_temperature ORDER BY time DESC LIMIT 10;
  ```
...
Telegraf extracts:
- `state` → `value` field (float)
- `unit_of_measurement` → `unit` tag
- `friendly_name` → `name` tag
- MQTT topic → `device_id` tag (via regex)

#### InfluxDB 3 Output (ESP sensors)
```toml
[[outputs.influxdb_v2]]
...
**Writes to (`temperature_data`):**
- **Database**: `homeassistant` (auto-created on first write if doesn't exist)
- **Measurement**: `esp_temperature` (ESP device temperatures)
...
### Example 1: Temperature Sensor (ESP direct)

**MQTT Message:**
```
Topic: homeassistant/sensor/bedroom_temp/state
Payload: {"state": "21.5", "attributes": {"unit_of_measurement": "°C", "friendly_name": "Bedroom Temperature"}}
```

**InfluxDB 3 Table Row (esp_temperature):**
```
celsius=23.31, fahrenheit=73.96, device=Big-Garage, chip_id=E098068CC2EC, host=telegraf, source=mqtt, topic=esp-sensor-hub/Big-Garage/temperature, time=2025-12-13T01:40:17.964120889
```

**Query:**
```sql
SELECT time, device, celsius, fahrenheit FROM esp_temperature WHERE device = 'Big-Garage' ORDER BY time DESC LIMIT 100;
```
...
**Example:** Topic `homeassistant/sensor/bedroom_temp_celsius/state` → tag `location=bedroom`
...
4. **Gradual migration**: Keep InfluxDB 2.7 for legacy data, new MQTT → v3
